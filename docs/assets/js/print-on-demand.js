...
class PrintOnDemandManager {
    ...
    async getStores() {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Printify Store API Error: ${response.status} - ${response.statusText}`);
            }
    ...
    async getStoreDetails(shopId) {
        try {
            ...
            return stores.find(store => store.id.toString() === shopId.toString()) || null;
        } catch (error) {
    ...
    async disconnectStore(shopId) {
        try {
            ...
            const response = await fetch(`${this.printifyApiUrl}/shops/${shopId}/connection.json`, {
                method: 'DELETE',
                headers: this.printifyHeaders
            });
    ...
    async getPrintifyProducts() {
        try {
            const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/products.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async getCatalogBlueprints() {
        try {
            ...
            const response = await fetch(`${this.printifyApiUrl}/catalog/blueprints.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async getBlueprintDetails(blueprintId) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/catalog/blueprints/${blueprintId}.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async getPrintProviders() {
        try {
            ...
            const response = await fetch(`${this.printifyApiUrl}/catalog/print_providers.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });

            if (!response.ok) {
                throw new Error(`Printify Print Providers API Error: ${response.status}`);
            }
    ...
    async getBlueprintPrintProviders(blueprintId) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/catalog/blueprints/${blueprintId}/print_providers.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });

            if (!response.ok) {
                throw new Error(`Printify Blueprint Providers API Error: ${response.status}`);
            }
    ...
    async getBlueprintVariants(blueprintId, printProviderId) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/catalog/blueprints/${blueprintId}/print_providers/${printProviderId}/variants.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async getShippingInfo(blueprintId, printProviderId) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/catalog/blueprints/${blueprintId}/print_providers/${printProviderId}/shipping.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async uploadImage(imageData) {
        try {
            ...
            const response = await fetch(`${this.printifyApiUrl}/uploads/images.json`, {
                method: 'POST',
                headers: this.printifyHeaders,
    ...
    async getWebhooks() {
        try {
            const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/webhooks.json`, {
                method: 'GET',
                headers: this.printifyHeaders
            });
    ...
    async createWebhook(webhookData) {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Printify Webhook Creation API Error: ${response.status}`);
            }
    ...
    async deleteWebhook(webhookId) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/webhooks/${webhookId}.json`, {
                method: 'DELETE',
                headers: this.printifyHeaders
            });

            if (!response.ok) {
                throw new Error(`Printify Webhook Deletion API Error: ${response.status}`);
            }
    ...
    async createPrintifyProduct(productData) {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Printify API Error: ${response.status} - ${errorData.message || response.statusText}`);
            }
    ...
    async updatePrintifyProduct(productId, productData) {
        try {
            const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/products/${productId}.json`, {
                method: 'PUT',
                headers: this.printifyHeaders,
                body: JSON.stringify(productData)
            });
    ...
    async publishPrintifyProduct(productId, publishOptions = {}) {
        try {
            ...
            const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/products/${productId}/publish.json`, {
                method: 'POST',
                headers: this.printifyHeaders,
    ...
    async getTShirtBlueprints() {
        try {
            ...
            const tshirtKeywords = ['tee', 't-shirt', 'shirt', 'cotton', 'crew', 'regular fit', 'heavy cotton'];
    ...
    async getLongSleeveBlueprints() {
        try {
            ...
            const longSleeveKeywords = ['long sleeve', 'long-sleeve', 'ls', 'longsleeve'];
    ...
    async getRecommendedTShirtProviders() {
        try {
            ...
            const qualityProviders = ['Printful', 'GOOTEN', 'SwiftPOD', 'Dream Junction'];
    ...
    async setupTShirtProduct(productConfig) {
        try {
            ...
            const blueprint = tshirtBlueprints.find(bp =>
            ...
            const printProviders = await this.getBlueprintPrintProviders(blueprint.id);
            ...
            const variants = await this.getBlueprintVariants(blueprint.id, provider.id);
            console.log('Available variants:', variants.variants?.length || 0);

            // 4. Create product data structure
            const productData = {
                title: productConfig.name,
                description: productConfig.description || `High-quality ${productConfig.name} printed on demand`,
                blueprint_id: blueprint.id,
                print_provider_id: provider.id,
                variants: this.formatVariantsForProduct(variants.variants, productConfig.price || 24.99),
                print_areas: productConfig.printAreas || this.getDefaultTShirtPrintAreas(productConfig.imageUrl)
            };
    ...
    async getAllProducts() {
        try {
            ...
            return {
                ...
                combined: this.combineProductCatalogs(printifyProducts, printfulProducts)
            };
        } catch (error) {
    ...
    shouldSyncWithPrintify(product) {
        ...
        return product.category === 't-shirts' || product.category === 'long-sleeve';
    }
    ...
    async createPrintifyOrder(orderData) {
        const response = await fetch(`${this.printifyApiUrl}/shops/${this.printifyShopId}/orders.json`, {
            method: 'POST',
            headers: this.printifyHeaders,
    ...
    // UTILITY METHODS
    isApiConfigured() {
        return (this.printifyApiKey && this.printifyApiKey !== 'YOUR_PRINTIFY_API_KEY') ||
    ...
    async validateConfiguration() {
        const validation = {
            printify: {
                configured: this.printifyApiKey && this.printifyApiKey !== 'YOUR_PRINTIFY_API_KEY_HERE',
                shopIdConfigured: this.printifyShopId && this.printifyShopId !== 'YOUR_SHOP_ID_HERE',
                connected: false,
                storeExists: false,
                storeName: null,
                error: null
            ...
            printful: {
                configured: this.printfulApiKey && this.printfulApiKey !== 'YOUR_PRINTFUL_API_KEY_HERE',
                connected: false,
                error: null
            },
            overall: {
        ...
        if (validation.printify.configured && validation.printify.shopIdConfigured) {
            try {
                const stores = await this.getStores();
                validation.printify.connected = true;

                const currentStore = stores.find(store => store.id.toString() === this.printifyShopId.toString());
                if (currentStore) {
        ...
        if (validation.printful.configured) {
            try {
                const products = await this.getPrintfulProducts();
        ...
        validation.overall.ready = validation.printify.configured &&
        ...
        if (!validation.overall.ready) {
            if (!validation.printify.configured) validation.overall.errors.push('Printify API key not configured');
            if (!validation.printify.shopIdConfigured) validation.overall.errors.push('Printify Shop ID not configured');
            if (!validation.printify.connected) validation.overall.errors.push('Cannot connect to Printify API');
            if (!validation.printify.storeExists) validation.overall.errors.push('Printify store not found');
        }
