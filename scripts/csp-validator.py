...
def validate_csp_policy(directives):
    ...
    for directive in directives:
        ...
        if "'unsafe-eval'" in values:
            issues.append(
                f"{directive_name} allows 'unsafe-eval' which can be dangerous"
            )

        if "'unsafe-inline'" in values and directive_name == "script-src":
            recommendations.append(
                f"Consider removing 'unsafe-inline' from {directive_name} and using nonces or hashes"
            )

        # Check for wildcard usage
        if "*" in values:
            recommendations.append(
                f"{directive_name} uses wildcard (*) - consider being more specific"
            )
...
def generate_report():
    ...
    print(
        f"Compliance rate: {(report['files_with_csp']/report['total_files']*100):.1f}%"
    )
